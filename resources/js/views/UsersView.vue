...
<template>
    ...
    <section class="panel">
        ...
        <div class="table-wrap"><table><thead><tr><th>User</th><th>Email</th><th>Role</th><th>Status</th><th>Action</th></tr></thead><tbody>
            <tr v-for="user in filtered" :key="user.id"><td data-label="User"><strong>{{ user.name }}</strong><small v-if="user.must_change_password">Password change required</small></td><td data-label="Email">{{ user.email }}</td><td data-label="Role"><select v-model="user.role" :disabled="user.id === auth.user.id" @change="role(user)"><option>admin</option><option>assistant</option><option>cashier</option><option>user</option></select></td><td data-label="Status"><span class="tag" :class="{ warn: !user.is_active }">{{ user.is_active ? 'Active' : 'Disabled' }}</span></td><td data-label="Action"><button class="btn tiny danger" :disabled="user.id === auth.user.id" @click="remove(user)">{{ user.id === auth.user.id ? 'Current account' : 'Remove access' }}</button></td></tr>
        </tbody></table></div>
    </section>
