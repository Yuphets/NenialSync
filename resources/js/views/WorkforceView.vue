...
<template>
    ...
    <section v-if="tab === 'payroll'" class="panel table-wrap"><div class="panel-head"><div><h2>Weekly payroll preview</h2><small>Select statutory deductions for each employee. Values recalculate immediately.</small></div><div class="actions"><button class="btn" :disabled="exporting" @click="downloadPayroll">{{ exporting ? 'Preparing…' : 'Download payroll CSV' }}</button><button class="btn primary" @click="run">Finalize payroll run</button></div></div><table><thead><tr><th>Employee</th><th>Deduction plan</th><th>Gross</th><th>SSS</th><th>Pag-IBIG</th><th>PhilHealth</th><th>Net</th><th v-if="auth.role === 'admin'">Action</th></tr></thead><tbody><tr v-for="row in preview" :key="row.employee.id"><td><strong>{{ row.employee.name }}</strong><small>{{ row.employee.job_title }}</small></td><td><details class="deduction-dropdown"><summary>{{ deductionLabel(row.employee.id) }}</summary><div class="deduction-options"><label v-for="deduction in deductions" :key="deduction.code"><input v-model="deductionDrafts[row.employee.id]" type="checkbox" :value="deduction.code"><span>{{ deduction.label }}</span></label><button class="btn tiny primary" :disabled="saving === row.employee.id" @click.prevent="saveDeductions(row, $event)">{{ saving === row.employee.id ? 'Saving…' : 'Apply deductions' }}</button></div></details></td><td>₱{{ row.calculation.gross_pay }}</td><td>₱{{ row.calculation.sss }}</td><td>₱{{ row.calculation.pagibig }}</td><td>₱{{ row.calculation.philhealth }}</td><td><strong>₱{{ row.calculation.net_pay }}</strong></td><td v-if="auth.role === 'admin'"><button class="btn tiny danger" @click="remove(row.employee)">Remove</button></td></tr></tbody></table></section>
    <section v-else class="panel table-wrap"><div class="panel-head"><h2>Facial attendance</h2><span class="live">● Device ready</span></div><table><thead><tr><th>Employee</th><th>Date</th><th>Status</th><th>Confidence</th><th v-if="auth.role === 'admin'">Manual fallback</th></tr></thead><tbody><tr v-for="record in attendance" :key="record.id"><td>{{ record.employee?.name }}</td><td>{{ record.attendance_date }}</td><td>{{ record.status }}</td><td>{{ record.match_confidence || '—' }}%</td><td v-if="auth.role === 'admin'"><button class="btn tiny" @click="mark(record.employee)">Mark present</button></td></tr></tbody></table></section>
