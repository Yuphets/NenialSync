...
<template>
    ...
    <section v-if="!data.customer_view" class="panel table-wrap"><div class="panel-head"><h2>Live inventory activity</h2><span class="live">● Monitoring</span></div><div v-if="!data.latest_movements?.length" class="empty">Inventory movements will appear here in real time.</div><table v-else><thead><tr><th>Time</th><th>Product</th><th>Type</th><th>Stock change</th><th>Reserved change</th></tr></thead><tbody><tr v-for="movement in data.latest_movements" :key="movement.id"><td>{{ new Date(movement.created_at).toLocaleString() }}</td><td>#{{ movement.product_id }}</td><td>{{ movement.type }}</td><td>{{ movement.quantity_delta }}</td><td>{{ movement.reserved_delta }}</td></tr></tbody></table></section>
</template>
